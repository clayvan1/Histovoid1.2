...
const OptionWheel = ({
  ...
  // Fix: Add ref to track wheel scrolling
  ...
  // Apply target - ONLY for visual scrolling, NOT for selection
  const applyTarget = useCallback(
    (value, snap) => {
      ...
      // Update visual selection but DO NOT trigger onChange
      const idx = ((Math.round(v) % cfg.count) + cfg.count) % cfg.count;
      if (idx !== selectedRef.current) {
        selectedRef.current = idx;
        setSelectedIndex(idx);
        // Only play sound, don't trigger onChange
        playTick();
      }
      startLoop();
    },
  ...
  // Handle click on item - THIS triggers onChange
  const handleItemClick = useCallback(
    index => {
      // Fix: Prevent click if wheel was just scrolling
      ...
      // TRIGGER onChange ONLY on click
  ...
  // Wheel / touchpad scrolling - updates visual but NOT selection
  useEffect(() => {
    const el = rootRef.current;
    if (!el) return;
    const onWheel = e => {
      e.preventDefault();
      // Fix: Mark that wheel is scrolling
      ...
      wheelTimerRef.current = setTimeout(() => {
        ...
        // Fix: Reset wheel scrolling flag after a delay
        setTimeout(() => {
          wheelScrollingRef.current = false;
        }, 100);
      }, 140);
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          onPointerUp={(e) => {
            // Fix: Use pointer up instead of click for better control
            e.stopPropagation();
